...
def find_ratio_for_target(factors_matrix: np.ndarray, target: float, iters: int = 44) -> float:
...
with st.sidebar:
    ...
    up_xlsx = st.file_uploader("Upload workbook with '12 Month Factor'", type=["xlsx", "xlsm", "xls"])

# Require the Excel workbook
if up_xlsx is None:
    st.info("Upload your Excel workbook to generate the matrix.")
    st.stop()

# Load 12 Month Factor series and determine valid starts
series = find_sheet_series(up_xlsx.getvalue())
...
# Precompute success thresholds per horizon (ratios)
sim_slices = [sim_factors[:, i:] for i in range(years)]
ratio_start  = np.zeros(years)  # 95% for Year 1 default
ratio_low    = np.zeros(years)  # 75%
ratio_high   = np.zeros(years)  # 90%
ratio_target = np.zeros(years)  # 80%
...
st.dataframe(wd_df.round(2), use_container_width=True)
...
row_avg_df = pd.DataFrame({
    "Percentile": pct_list,
    "Row Avg Withdrawal ($)": np.round(row_avg_pcts, 2),
})
