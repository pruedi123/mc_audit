...
with st.sidebar:
    ...
    alloc_choice = st.selectbox(
        "Equity Allocation",
        options=lbm_cols if lbm_cols else ["(global_factors.csv not found or no LBM columns)"]
    )
...
st.subheader("Global Factors (CSV)")

if df_global is None or not lbm_cols:
    st.error("Could not load 'global_factors.csv' with LBM columns. Please add the file with columns like 'LBM 60E', 'LBM 100F', etc.")
    st.stop()

# Use the selected allocation column as the historical path source
series = pd.to_numeric(df_global[alloc_choice], errors='coerce').dropna().reset_index(drop=True)
