...
if data_choice.startswith("Both"):
    ...
    for label, mat, df in mats:
        df_fmt = df.applymap(lambda x: f"${x:,.0f}")
        st.subheader(
            f"Withdrawals Matrix — {label} (Years as Columns) — Start {int(round(start_success*100))}%, "
            f"Adjust to {int(round(target_success*100))}% if >{int(round(high_thr*100))}% or <{int(round(low_thr*100))}%"
        )
    ...
    # Also compute percentiles and below-Year1 stats for each
    for label, mat, df in mats:
        ...
        st.subheader(f"Row‑Average Withdrawals — Percentiles ({label})")
        st.dataframe(row_avg_df, use_container_width=True)
        st.download_button(
            f"Download row_avg_withdrawal_percentiles_{label.lower()}.csv",
            data=row_avg_df.to_csv(index=False).encode(),
            file_name=f"row_avg_withdrawal_percentiles_{label.lower()}.csv",
            mime="text/csv",
        )
