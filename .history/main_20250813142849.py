...
# --- Percentiles of ending balances (EOY {years}) ---
end_vals = np.zeros(n_valid, dtype=float)
for r in range(n_valid):
    bal = float(start_balance)
    for y in range(years):
        factor = float(series.iloc[r + stride * y])
        if y == 0 and not use_year1_factor:
            factor = 1.0
        bal = (bal - withdraw_mat[r, y]) * factor
    end_vals[r] = bal

end_pcts = np.percentile(end_vals, pct_list)
end_df = pd.DataFrame({
    "Percentile": [f"{p}%" for p in pct_list],
    "Ending Balance ($)": [f"${v:,.0f}" for v in end_pcts],
})

st.subheader(f"Ending Balances — Percentiles (EOY {years})")
st.dataframe(end_df, use_container_width=True)

st.download_button(
    "Download ending_balance_percentiles.csv",
    data=end_df.to_csv(index=False).encode(),
    file_name="ending_balance_percentiles.csv",
    mime="text/csv",
)
